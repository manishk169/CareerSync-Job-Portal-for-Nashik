...
if (isset($_GET['applicant_email']) && isset($_GET['company_name']) && isset($_GET['applicant_name']) && isset($_GET['job_title']) && isset($_GET['job_id'])) {
    ...
    $stmt = $conn->prepare("UPDATE applications SET application_status = 'rejected' WHERE js_username = ? AND job_id = ?");
    if ($stmt === false) {
    ...
    if ($execute) {
        ...
        try {
            ...
            $mail->Body    = "Your application for the post of $job_title in $company_name has been rejected. We wish you success in your ongoing job search. Thank you for your interest in our company. Best wishes.";

            $mail->send();
            echo 'Message has been sent';
            echo "<script>alert('Job Application Rejected..');
